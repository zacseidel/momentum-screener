...
def format_html_email(top10_df, report_date=None):
    ...
    if all(date in voo for date in voo_dates.values()):
        ...
        voo_1wreturn = (voo_now / voo_week_ago - 1) if voo_then > 0 else None
        ...
        voo_ret1w_fmt = f"+{voo_1wreturn:.1%} last week" if isinstance(voo_1wreturn, (float, int)) else "— last week"
        voo_line = f"<p><strong>Benchmark (VOO):</strong> {voo_price_fmt} ({voo_ret_fmt}, {voo_ret1w_fmt})</p>"
    else:
    ...
    for _, row in top10_df.iterrows():
        ...
        enriched.append({
            ...
            "last_week_return": row["last_week_return"],
    ...
    for ticker in tickers:  # maintain order
        ...
        ret_val_12m  = top10_df.loc[… , "current_return"].values[0]
        ret_val_wk   = top10_df.loc[… , "last_week_return"].values[0]
        price_val = prices.get(ticker, "—")
        ...
        try:
            ...
            ret12_fmt = f"+{float(ret_val_12m):.1%} last 12M"
            retwk_fmt = f"+{float(ret_val_wk):.1%} last week"
        except:
            #ret_fmt = f"{ret_val} last 12M"
            ret12_fmt = f"+{ret_val_12m} last 12M, "
            retwk_fmt = f"+{ret_val_wk} last week"

        text = f"{ticker} – {price_fmt} ({ret12_fmt}, {retwk_fmt})"
        if ticker in added:
    ...
    summary_html = voo_line + "<h3>Summary of Changes</h3><p>" + "<br>".join(summary_lines) + "</p><br><br><p>New stocks in <b>bold</b>, dropped stocks in <s>strikethrough</s></p>"
