...
def format_html_email(top10_df, report_date=None):
    """
    Backwards‑compatible single‑index e‑mail (SPY‑only).  Delegates to
    the new dual‑index engine with an empty MDY dataframe.
    """
    empty = pd.DataFrame(columns=top10_df.columns)
    return format_dual_html_email(top10_df, empty, report_date)

def format_dual_html_email(top10_spy_df, top10_mdy_df, report_date=None):
    """
    HTML e‑mail with hierarchy:
        Summary:
          - SPY Summary
          - MDY Summary
        Enriched:
    ...
    return page_tpl.render(
        ...
        mdy_summary=mdy_summary,
        spy_cards=spy_cards,
        mdy_cards=mdy_cards,
    )

# ---------------------------------------------------------------------------
# Private helper: build the “summary” block and the 10 detailed cards
# for ONE index (SPY or MDY).  All heavy lifting—DB queries, chart
# generation, style_return(), etc.—is exactly the same logic you
# already had in format_html_email, but we now return two HTML
# strings instead of a full page.
# ---------------------------------------------------------------------------
def _build_index_sections(top10_df, index_label="SPY", report_date=None):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        # ------------------------------------------------------------------
        table_map = {"SPY": "top10_picks", "MDY": "top10_mdy"}
...
# -----------------------------------------------------------------
    ...
    summary_html = voo_line + f'<h3 id="{summary_id}">Summary of Changes ({index_label})</h3><p>' + "<br>".join(summary_lines) + "</p><p>New stocks in <i><span style=\"color:#0000FF\">blue italic</span></i>, dropped stocks in <span style=\"color:#808080\">gray</span></p>"
